...
var WeiboSinaFactory = {
    create : function(username, password, size, config, callback){
        phantom.create('--cookies-file=cookie2.txt', function(ph){
            ...
            //console.log(pages);
            async.map(pages, function(index, cb){
                ph.createPage(function(page){
                    page.set('settings.loadImages', false);
                    console.log('New WeiboSina:'+index);
                    cb(null, new WeiboSina(index, username, password, page, ph, config));
                });
            },
            function(err, results){
                // results is now an array of stats for each file
                //console.log('00000000000000000');
                callback(new PageUpstream(results));
            });
        });
    }
};
...
function WeiboSina(page_number, username, password, page, ph, config) {
    ...
    _self.open = function(url, callback){
        if(_is_login==true && _is_openning==false) {
            ...
            _openPage(url, callback, run_next);
        }
        else {
            _log('加入队列:'+url);
    ...
    function _openPage(url, userCallback, nextPageCallback) {
        var is_call = false;
        ...
        page.open(url, function(status) {
            if(status=='success') {
                ...
                //注入jquery
                page.injectJs('script/jquery-1.7.2.min.js',function(info){
                    _log('injectJs:'+info)
